...
Auto-generates localized themes for the playable ad. One brief (`tools/theme-gen/briefs/<id>.json`) → a complete theme: collectible icons and field decor in `src/assets/themes/<id>/`, a parametric SVG background, `src/themes/generated/<id>.js`, `.env.<id>`, and a `build:<id>` script in `package.json`. During `theme:gen`, a report is written under `tools/theme-gen/runs/<timestamp>/<id>/report.md` (screenshots, QA verdict); that tree and `tools/theme-gen/agent-stage/` are local-only (listed in `.gitignore`). A consolidated table of theme ids and human-readable names (including hand-authored themes) lives in [`themes.md`](themes.md).
